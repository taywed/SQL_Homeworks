...
-- 1
SELECT CAST(emp_id AS VARCHAR) + '-' + first_name + ' ' + last_name AS employee_info
FROM employees
WHERE emp_id = 100;
...
SELECT year,
       (SELECT MAX(val) 
        FROM (VALUES (Max1), (Max2), (Max3)) AS Value(val)) AS MaxValue
FROM TestMax;
...
SELECT emp_id, first_name, last_name, hire_date,
       ROUND(DATEDIFF(DAY, hire_date, GETDATE()) / 365.0, 2) AS YearsOfService
FROM employees
...
SELECT
    ...
    STRING_AGG(CASE WHEN value LIKE '[^a-zA-Z0-9]' THEN value ELSE '' END, '') AS Others
FROM chars;
...
FROM Ungroup
CROSS APPLY (
    SELECT TOP (quantity) 1 AS n FROM master.dbo.spt_values
) AS x;

-- 15
SELECT some_column,
       STRING_AGG(another_column, ', ') AS concatenated_values
FROM DMLTable
...
SELECT department_id,
       ...
       SUM(CASE WHEN DATEDIFF(YEAR, hire_date, GETDATE()) > 3 THEN 1 ELSE 0 END) * 100.0 / COUNT(*) AS percent_over_3_years
FROM employees
GROUP BY department_id;

-- 20
SELECT job_description,
       MIN(hire_date) AS MostExperienced,
       MAX(hire_date) AS LeastExperienced
FROM Personal
...
SELECT id, value + prev_value AS new_value
FROM Ordered;
...
-- 23
-- Note: SQL Server does not support direct evaluation of math strings.
-- You need dynamic SQL or CLR integration. Placeholder syntax:
-- SELECT equation, result FROM Equations

-- 24
SELECT s1.student_id, s1.name, s1.birthday
FROM Student s1
JOIN (
    SELECT birthday
    FROM Student
